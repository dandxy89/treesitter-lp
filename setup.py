...
class Build(build):
    def run(self):
        dest = path.join(self.build_lib, "tree_sitter_lp", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_lp": ["*.pyi", "py.typed"],
        "tree_sitter_lp.queries": ["*.scm"],
    },
    ext_package="tree_sitter_lp",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_lp/binding.c",
                "src/parser.c",
                "src/scanner.c",
            ],
            define_macros=[
